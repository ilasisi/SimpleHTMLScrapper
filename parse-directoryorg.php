...
$html = file_get_html('https://www.directory.org.ng/category-photography_video_services');

// find all the rows of the table
$rows = $html->find('div[class="listing-firm"]');
...
if(count($rows)>1){
	...
	for($i=1; $i<count($rows); $i++){
		...
		$address = $rows[$i]->find('address',0)->plaintext;
		$business_url = $rows[$i]->find('h4 a',0)->href;
        
        $html2 = file_get_html('https://www.directory.org.ng/'.$business_url);
        $phone_no = $html2->find('div[class="listing-firm"] fieldset p',0)->plaintext;
